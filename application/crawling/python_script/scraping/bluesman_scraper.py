...
class BluesmansScraper:
    ...
    def scrap(self):
        ...
        page_no = 1
        while True:
            items = BluesmansScraper.scrap_new_arrival_item(self, page_no)
    ...
    def scrap_new_arrival_item(self, page_no):
        ...
        for item in soup.find_all('div', {'class': 'col xans-record-'}):
            data.append(BluesmansScraper.__generate_dict_item(self, item))

        print('complete scraping: ' + url)

        return data

    def __generate_dict_item(self, item):
        item_dict = dict()
